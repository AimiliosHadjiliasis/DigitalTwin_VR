Shader "Unlit/HeatmapShader"
{
	Properties
	{
	  ...
	  _Color0("Color 0",Color) = (0,0,0,1)
		...
		_Diameter("Diameter",Range(0,10)) = 10.0
		...
		SubShader
	  {
		...
		Pass
		{
		  ...
		  float3 getHeatForPixel(float weight)
		  {
			  ...
			  else if (weight > pointranges[1] && weight < pointranges[2])
			  {
				  return colors[1];
			  }
			  else if (weight >= pointranges[2])
			  {
				  if (_ColorIndex == 1)
					  return colors[2];
				  else if (_ColorIndex == 2)
					  return colors[3];
				  else if (_ColorIndex == 3)
					  return colors[4];
			  }
